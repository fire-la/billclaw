 ...
 * Run config command
 */
async function runConfig(context: CliContext, args?: { key?: string; value?: string; list?: boolean }): Promise<void> {
 ...
 * Get a config value
 */
async function getConfig(runtime: CliContext["runtime"], key: string): Promise<void> {
  const config = await runtime.config.getConfig();
 ...
 * Set a config value
 */
async function setConfig(runtime: CliContext["runtime"], key: string, valueStr: string): Promise<void> {
  const config = await runtime.config.getConfig();
...
export const configCommand: CliCommand = {
  ...
  handler: (context: CliContext, args?: Record<string, unknown>) => {
    const typedArgs = args as { key?: string; value?: string; list?: boolean } | undefined;
