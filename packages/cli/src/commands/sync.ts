 ...
 * Run sync command
 */
async function runSync(context: CliContext, args: { account?: string; all?: boolean }): Promise<void> {
 ...
 */
async function syncSingleAccount(billclaw: Billclaw, accountId: string): Promise<void> {
  ...
  try {
    ...
    spinner.succeed(
      `Synced ${accountId}: +${added} new, ${updated} updated`
    );
...
export const syncCommand: CliCommand = {
  ...
  handler: (context, args) => runSync(context, args as { account?: string; all?: boolean }),
};
