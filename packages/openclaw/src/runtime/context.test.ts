...
describe("OpenClawLogger", () => {
  ...
  describe("info", () => {
    ...
    it("should handle multiple arguments", () => {
      ...
      expect(mockApi.logger.info).toHaveBeenCalledWith("message", "arg2", {
        key: "value",
      })
    })
...
describe("OpenClawRuntimeContext", () => {
  ...
  it("should have readonly properties", () => {
    expect(Object.getOwnPropertyDescriptor(context, "logger")?.writable).toBe(
      false,
    )
    expect(Object.getOwnPropertyDescriptor(context, "config")?.writable).toBe(
      false,
    )
    expect(Object.getOwnPropertyDescriptor(context, "events")?.writable).toBe(
      false,
    )
  })
})
