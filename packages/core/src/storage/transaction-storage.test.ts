...
describe("initializeStorage", () => {
  ...
  it("should create storage directory structure", async () => {
    ...
    expect(await fs.access(transactionsDir).then(() => true).catch(() => false)).toBe(true);
    expect(await fs.access(accountsDir).then(() => true).catch(() => false)).toBe(true);
    expect(await fs.access(syncDir).then(() => true).catch(() => false)).toBe(true);
  });
});
